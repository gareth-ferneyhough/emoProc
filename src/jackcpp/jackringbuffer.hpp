...
// Modified March 2012 by Gareth Ferneyhough, University of Nevada Reno.
// I added peek(), and erase() methods.
  ...
  */
  ...
  public:
    ...
    */
    ...
    /**
       @brief Peek into an array

       Read from the buffer into an array. No elements removed from the buffer.

       \param dest an array to be read into
       \param cnt the number of elements to read into this array
    */

    void peek(Type *dest, unsigned cnt){
      jack_ringbuffer_data_t readVec[2];
      unsigned int read_size = sizeof(Type) * cnt;
      if(getReadSpace() <= 0){
        //throw error!!!!
    ...
    /**
       @brief Erase elements from front of buffer.

       \param cnt the number of elements to erase
    */
    void erase(unsigned int cnt){
      unsigned int read_size = sizeof(Type) * cnt;
      jack_ringbuffer_read_advance(mRingBufferPtr, read_size);
    }
